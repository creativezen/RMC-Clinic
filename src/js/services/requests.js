...
function appointmentRequest(session) {
	...
	let buttonAppointmentArray = Array.from(
		document.querySelectorAll('.js-button-appointment')
	)
	let selectAppointmentDirection = document.querySelector(
		'.js-select-appointment[data-select="direction"]'
	)
	let selectAppointmentService = document.querySelector(
		'.js-select-appointment[data-select="service"]'
	)
	...
	buttonAppointmentArray?.forEach(function (button) {
		...
		button.addEventListener('click', function (e) {
			if (
				this.dataset.idDirection !== undefined &&
		...
		selectAppointmentDirection?.addEventListener('change', e => {
			...
			if (e.target.options[e.target.selectedIndex].value !== 'Не выбрано') {
				...
				function actionResponce(responce) {
					selectAppointmentService.disabled = false
					selectAppointmentService.innerHTML = responce
					// selectAppointmentService.innerHTML = `<option>Услуга</option>` // Для отладки
				}
			} else {
		...
		Array.from(selectAppointmentDirection.options)?.forEach(option => {
			if (option.dataset.id && option.dataset.id == idDirection) {
				...
				function actionResponce(responce) {
					...
					selectAppointmentService.innerHTML = responce
					setCurrentOption(
						Array.from(selectAppointmentService.options),
						idService
					)
				}
